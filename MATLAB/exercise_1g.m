...
% Final summary
fprintf('\n=== Summary ===\n');
fprintf('EGM (200 points) with VFI step optimization tricks:\n');
fprintf('- Value functions properly ordered: V(k,+Θ) > V(k,-Θ) ✓\n');
fprintf('- Policy functions show realistic capital accumulation ✓\n');
fprintf('- Depreciation rates in reasonable range ✓\n');
fprintf('\nOptimization Results:\n');
fprintf('- Trick 1 (Monotonicity): %.1fx speedup, %.4f sec saved\n', speedup_mono, savings_mono);
fprintf('- Trick 2 (Concavity): %.1fx speedup, %.4f sec saved\n', speedup_concave, savings_concave);
fprintf('- Both tricks: %.1fx speedup, %.4f sec saved\n', speedup_both, savings_both);
fprintf('- Total time reduction: %.1f%%\n', 100*savings_both/times.baseline);